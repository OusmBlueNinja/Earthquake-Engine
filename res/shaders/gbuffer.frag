...
{
    if ((u_MaterialTexMask & MAT_TEX_HEIGHT) == 0)
        return uv;
...
void main()
{
    ...
    if (u_AlphaTest == 1 && opacity < u_AlphaCutoff)
        discard;
    ...
    if ((u_MaterialTexMask & MAT_TEX_ROUGHNESS) != 0)
        rough = clamp(texture(u_RoughnessTex, uv).r, 0.0, 1.0);
    ...
    oNormal = vec4(normalize(N) * 0.5 + 0.5, 1.0);
    oMaterial = vec4(rough, metal, ao, 0.0);
