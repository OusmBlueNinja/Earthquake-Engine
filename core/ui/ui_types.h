...
typedef struct ui_vec2_t
{
    float x, y;
} ui_vec2_t;
typedef struct ui_vec3_t
{
    float x, y, z;
} ui_vec3_t;
typedef struct ui_vec4_t
{
    float x, y, z, w;
} ui_vec4_t;
typedef struct ui_vec2i_t
{
    int32_t x, y;
} ui_vec2i_t;
...
static inline ui_vec2_t ui_v2(float x, float y)
{
    ui_vec2_t v;
    v.x = x;
    v.y = y;
...
static inline ui_vec3_t ui_v3(float x, float y, float z)
{
    ui_vec3_t v;
    v.x = x;
    v.y = y;
...
static inline ui_vec4_t ui_v4(float x, float y, float z, float w)
{
    ui_vec4_t v;
    v.x = x;
    v.y = y;
...
static inline ui_vec2i_t ui_v2i(int32_t x, int32_t y)
{
    ui_vec2i_t v;
    v.x = x;
    v.y = y;
...
{
    if (a->count + 1 > a->cap)
        if (!ui_array_reserve(a, a->count + 1))
            return 0;
