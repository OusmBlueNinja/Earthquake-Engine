...
static inline ecs_entity_t ecs_vec_ent_get(const vector_t *v, uint32_t i)
{
    return *(ecs_entity_t *)((uint8_t *)v->data + (size_t)i * v->element_size);
}

static inline void ecs_vec_ent_set(vector_t *v, uint32_t i, ecs_entity_t x)
{
    *(ecs_entity_t *)((uint8_t *)v->data + (size_t)i * v->element_size) = x;
}
