...
{
    if (!src || !out || !src->vertices || !src->indices)
        return 0;
    if (src->vertex_count < 3u || src->index_count < 3u)
        return 0;
    ...
    if (!size_mul_ok((size_t)src->vertex_count, sizeof(model_vertex_t), &vb))
        return 0;
    if (!size_mul_ok((size_t)src->index_count, sizeof(uint32_t), &ib))
        return 0;
...
static int map_init(u64u32_map_t *m, uint32_t desired)
{
    ...
    size_t kb = 0, vb = 0;
    if (!size_mul_ok((size_t)cap, sizeof(uint64_t), &kb))
        return 0;
    if (!size_mul_ok((size_t)cap, sizeof(uint32_t), &vb))
        return 0;
    ...
    *out_loop_count = 0u;
    ...
    for (uint32_t t = 0; t < tri; ++t)
    {
        ...
        for (int e = 0; e < 3; ++e)
        {
            ...
            int ins = 0;
            uint32_t cur = 0u;
            if (!map_get_or_insert(&ecount, k, &cur, &ins))
            {
                map_free(&ecount);
                return 0;
            }
            ...
            {
                val = cur + 1u;
                if (!map_get_or_insert(&ecount, k, &val, &ins))
                {
                    map_free(&ecount);
                    return 0;
                }
    ...
    size_t nb = 0;
    if (!size_mul_ok((size_t)bcap, sizeof(adj_node_t), &nb))
    {
        map_free(&ecount);
        return 0;
    }
    ...
    for (uint32_t t = 0; t < tri; ++t)
    {
        ...
        for (int e = 0; e < 3; ++e)
        {
            ...
            uint32_t cnt = 0u;
            int ins = 0;
            if (!map_get_or_insert(&ecount, k, &cnt, &ins))
                continue;
            if (ins)
                continue;
            if (cnt != 1u)
            ...
            if (bcount + 2u > bcap)
            {
                ...
                size_t nb2 = 0;
                if (!size_mul_ok((size_t)new_cap, sizeof(adj_node_t), &nb2))
                    break;
    ...
    for (uint32_t vi = 0; vi < vcount; ++vi)
    {
        ...
        for (uint32_t n = n0; n != UINT32_MAX; n = nodes[n].next)
        {
            ...
            for (uint32_t steps = 0; steps < 1u << 30; ++steps)
            {
                ...
                if (best_to == UINT32_MAX || best_eid == UINT32_MAX)
                    break;
            ...
            if (flat_count - loop_starts[loop_count] >= 3u)
                loop_count++;
            else
                flat_count = loop_starts[loop_count];
        }
    }
...
{
    if (!out || !out->vertices || !out->indices)
        return;
...
static float lod_effective_total_ratio(const model_lod_settings_t *s, uint8_t li, uint8_t lod_count)
{
    ...
    if (total <= 0.0f)
        total = powf(0.5f, (float)li);
    total = clamp01(total);
    if (total < 0.001f)
        total = 0.001f;
    if (li == (uint8_t)(lod_count - 1u) && total > 0.5f)
        total = 0.5f;
    return total;
}
...
{
    memset(out, 0, sizeof(*out));
    if (!src || !src->vertices || !src->indices)
        return 0;
    if (src->vertex_count < 3u || src->index_count < 3u)
        return 0;
    ...
    size_t idxb0 = 0;
    if (!size_mul_ok((size_t)icount0, sizeof(uint32_t), &idxb0))
        return 0;
    ...
    for (uint32_t t = 0; t < tri0; ++t)
    {
        ...
        for (int e = 0; e < 3; ++e)
        {
            ...
            int ins = 0;
            uint32_t prev = 0u;
            if (!map_get_or_insert(&edge_counts, k, &prev, &ins))
                continue;
            if (!ins)
            {
                cur = prev + 1u;
    ...
    for (uint32_t t = 0; t < tri0; ++t)
    {
        ...
        for (int e = 0; e < 3; ++e)
        {
            ...
            uint32_t cnt = 0u;
            int ins = 0;
            if (!map_get_or_insert(&edge_counts, k, &cnt, &ins))
                continue;
            if (!ins && cnt == 1u)
            {
                p = 1u;
            ...
            if (ndot_edge(va, vb) < sharp_ndot)
            {
                p = 1u;
                break;
            }
            if (uv_seam_edge(va, vb, uv_max))
            {
                p = 1u;
                break;
            }
    ...
    if (keep_target < keep_forced)
        keep_target = keep_forced;
    ...
    for (uint32_t li = 0; li < loop_count; ++li)
    {
        ...
        uint32_t step = 1u;
        if (n > max_loop_edges)
            step = (uint32_t)ceilf((float)n / (float)max_loop_edges);
        if (step < 1u)
            step = 1u;
        ...
        for (uint32_t k = step; k < n; k += step)
        {
            ...
            if (cur >= out_vcount || prev >= out_vcount)
            {
                prev = cur;
                continue;
            }
...
bool model_raw_generate_lods(model_raw_t *raw, const model_lod_settings_t *s)
{
    ...
    if (min_lods == UINT32_MAX)
        min_lods = 1u;
    if (min_lods < 1u)
        min_lods = 1u;
    if (min_lods > (uint32_t)MODEL_LOD_MAX)
        min_lods = MODEL_LOD_MAX;
    ...
    for (uint32_t smi = 0; smi < raw->submeshes.size; ++smi)
    {
        ...
        if (l0 && l0->indices)
            tri0 = (l0->index_count / 3u);
        if (lN && lN->indices)
            triN = (lN->index_count / 3u);
